...
def add_poster(df):
    for i, row in tqdm(df.iterrows(), total=df.shape[0]):
        tmdb_id = row["tmdbId"]
        tmdb_url = f"https://api.themoviedb.org/3/movie/{tmdb_id}?api_key=12f7f88dfa748cf62be62f1ae86e981f"
        result= requests.get(tmdb_url)
        try:
            df.at[i, "poster_path"]="https://image.tmdb.org/t/p/original"+ result.json()['poster_path']
            time.sleep(0.1)
        except(TypeError, KeyError) as e :
            df.at[i, "poster_path"] = "https://image.tmdb.org/t/p/original/uXDfJdbP4ijW5hWSBrPrlKpxab.jpg"
